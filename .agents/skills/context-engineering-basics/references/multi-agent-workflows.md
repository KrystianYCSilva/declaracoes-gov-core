...
description: |
  Reference for multi-agent context management patterns: blackboard (shared state), context isolation by role, handoff compression protocols, and router and linear-chain workflow architectures.
  Use when: multiple agents must coordinate without leaking irrelevant context, or when an agent handoff must preserve the minimum necessary state.
---
